...
export function getDatabaseFromRegistration(registration) {
  ...
  // Final SOET fallback: read the campus digit by position rather than by searching
  // for "10"/"30" anywhere in the institute code.
  //
  // In the SOET series the campus digit sits at index 3: 0101... (PKD) and 0301...
  // (BBSR). Substring matching used to misread institute codes that merely contain
  // those digits - 253001320002 (a PKD B.Tech Phyto Pharma student, institute code
  // 3001) matched the "30" inside 3001 and was sent to the BBSR database, so the
  // student found no results when checking their own.
  //
  // An institute code that is neither falls through to the default below rather than
  // being guessed at.
  if (regStr[3] === '1') {
    return 'CUTMPKD'; // PKD SOET
  } else if (regStr[3] === '3') {
  ...
  // SOM Detection (Format: YY IIII BB SSSS -> e.g., 23 0109 12 0046)
  // Institute: indices 2–5 (4 digits); branch: indices 5–7 (912=BBA, 214=MBA)
  if (regStr.length === 12) {
    const instFour = regStr.slice(2, 6);
    const somBranch = regStr.slice(5, 8);
    if (somBranch === "912" || somBranch === "214") {
      if (instFour.startsWith("01")) return "CUTMSOMPKD";
      if (instFour.startsWith("02")) return "CUTMSOMBBSR";
      return "CUTMSOMPKD";
    }
  }
 ...
 * Based on indices 3-6 or 4-7 of registration number, or 12-char SOM format
 *
 * @param {string} registration - Student registration number
 * @returns {string|null} School name (SOET, SOVET, SOM) or null
 */
export function getSchoolFromRegistration(registration) {
  ...
  // SOM (MBA/BBA): 12 chars, branch at indices 5–7 (214=MBA, 912=BBA)
  if (regStr.length === 12) {
    const somBranch = regStr.slice(5, 8);
    if (somBranch === '912' || somBranch === '214') {
      return 'SOM';
  ...
  // Need at least 7 characters for SOET/SOVET patterns
  if (regStr.length < 7) {
    return null;
  }
...
/**
 * SOM program (BBA vs MBA) from 12-digit registration. Branch at indices 5–7: 912=BBA, 214=MBA.
 * @returns {'BBA'|'MBA'|null}
 */
export function getSomProgramFromRegistration(registration) {
  const regStr = String(registration || "").trim().toUpperCase();
  if (regStr.length !== 12) return null;
  const branchCode = regStr.slice(5, 8);
  if (branchCode === "912") return "BBA";
  if (branchCode === "214") return "MBA";
  return null;
}

/**
 * SOM BBA/MBA: MongoDB names to try in order (primary campus from reg, then the other).
 * Use when student data may exist in a different campus DB than the reg suggests.
 * @returns {string[]}
 */
export function getSomDatabaseSearchOrder(registration) {
  const regStr = String(registration || "").trim().toUpperCase();
  if (regStr.length !== 12) return [];
  const branch = regStr.slice(5, 8);
  if (branch !== "912" && branch !== "214") return [];
  const instFour = regStr.slice(2, 6);
  const pkd = "CUTMSOMPKD";
  const bbsr = "CUTMSOMBBSR";
  if (instFour.startsWith("01")) return [pkd, bbsr];
  if (instFour.startsWith("02")) return [bbsr, pkd];
  return [pkd, bbsr];
}

/**
 * DBs to query for logged-in SOM students (PKD/BBSR mismatch). Non-SOM returns single DB from registration.
 * @returns {string[]}
 */
export function getSomStudentDatabaseCandidates(registration) {
  const reg = String(registration || "").trim().toUpperCase();
  const ordered = getSomDatabaseSearchOrder(reg);
  if (ordered.length > 0) return ordered;
  if (getSchoolFromRegistration(reg) === "SOM") {
    return ["CUTMSOMPKD", "CUTMSOMBBSR"];
  }
  return [getDatabaseFromRegistration(reg)];
}

/**
 * Resolve school for user-panel APIs when registration pattern is ambiguous or missing.
 * @param {string|null|undefined} registration
 * @param {string|null|undefined} jwtSchool - from auth payload (e.g. som, soet)
 * @returns {'SOET'|'SOVET'|'SOM'|null}
 */
export function resolveUserPanelSchool(registration, jwtSchool) {
  const fromReg = getSchoolFromRegistration(registration);
  if (fromReg) return fromReg;
  if (!jwtSchool) return null;
  const j = String(jwtSchool).toLowerCase();
  if (j === 'som') return 'SOM';
  if (j === 'sovet' || j === 'sov') return 'SOVET';
  if (j === 'soet' || j === 'soe') return 'SOET';
  return null;
}

/**
 * Base path for user result/semester/backlog APIs (/api/soet | /api/sovet | /api/som).
 * @param {'SOET'|'SOVET'|'SOM'|null|undefined} schoolUpper
 */
export function getUserSchoolApiBase(schoolUpperOrNull) {
  if (schoolUpperOrNull === 'SOVET') return '/api/sovet';
  if (schoolUpperOrNull === 'SOM') return '/api/som';
  return '/api/soet';
}
