...
export function getDatabaseFromRegistration(registration) {
  ...
  // Index 2-3: Institute Code (01=PKD, 02=BBSR)
  // Index 5-7: Branch Code (912=BBA, 214=MBA)
  if (regStr.length === 12) {
    const instCode = regStr.slice(2, 4);
    const somBranch = regStr.slice(5, 8);
    if (somBranch === '912' || somBranch === '214') {
      if (instCode === '01') return 'CUTMSOMPKD';
      if (instCode === '02') return 'CUTMSOMBBSR';
    }
  }
