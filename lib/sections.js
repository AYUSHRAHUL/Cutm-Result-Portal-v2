/**
 * Section allotment - which section a student belongs to within their branch and
 * batch.
 *
 ...
 *
 *   section_definitions  { branchKey, branch, batch, sections: ["A","B",...],
 *                          updatedAt, updatedBy }
 *       The section names that exist for one branch + batch. Set per branch and
 *       batch, so ECE 2025 can have A-C while CSE 2025 has A-F.
 *
 *   student_sections     { reg, section, branchKey, batch, updatedAt, updatedBy }
 *       One entry per student. Permanent for the programme, but can be changed.
 *
 * branchKey is always normalizeBranchKey() of the branch, never the raw label, so
 * "CSE AIML" and "AIML" cannot end up as two separate section lists.
 *
 * A student's section is only honoured while it still matches their *current*
 * branch and batch (after any branch override). If an admin later moves the
 * student to another branch, the old section belongs to the old branch, so they
 * read as Unassigned in the new one and are flagged for reassignment.
 */
...
/** Canonical key for a branch, used for all section storage and comparison */
export function sectionBranchKey(branch) {
  return normalizeBranchKey(branch);
}

/**
 * Section names defined for one branch + batch, in their saved order.
 */
export async function loadSectionDefinitions(db, branch, batch) {
  const branchKey = sectionBranchKey(branch);
  const batchYear = normalizeBatch(batch);
  if (!branchKey || !batchYear) return [];

  const doc = await db
    .collection("section_definitions")
    .findOne({ branchKey, batch: batchYear }, { projection: { _id: 0, sections: 1 } });
 ...
 *
 * @returns {Promise<Map<string, {section: string, branchKey: string, batch: string}>>}
 */
export async function loadStudentSections(db) {
  ...
  try {
    const docs = await db
      ...
      .find({}, { projection: { _id: 0, reg: 1, section: 1, branchKey: 1, batch: 1 } })
      .toArray();
    for (const d of docs) {
      const reg = String(d.reg || "").trim().toUpperCase();
      if (!reg || !d.section) continue;
      map.set(reg, { section: d.section, branchKey: d.branchKey || null, batch: d.batch || null });
    }
  } catch (e) {
...
/**
 * The section a student is in, given their CURRENT effective branch and batch.
 *
 * @returns {{ section: string|null, stale: boolean, previous: object|null }}
 *   section  - the section to use, or null if unassigned
 *   stale    - true when a section is stored but was allotted under a different
 *              branch or batch (e.g. the student was moved by a branch override)
 *   previous - the stored entry when stale, so the page can say what it was
 */
export function resolveSection(reg, effectiveBranch, effectiveBatch, sectionsMap) {
  const entry = sectionsMap?.get(String(reg || "").trim().toUpperCase());
  if (!entry) return { section: null, stale: false, previous: null };

  const branchKey = sectionBranchKey(effectiveBranch);
  const batchYear = normalizeBatch(effectiveBatch);

  const matches =
    (!entry.branchKey || entry.branchKey === branchKey) &&
    (!entry.batch || entry.batch === batchYear);

  return matches
    ? { section: entry.section, stale: false, previous: null }
    : { section: null, stale: true, previous: entry };
}
