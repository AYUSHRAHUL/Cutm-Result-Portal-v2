...
/**
 * SOM Students Route - SOM (BBA/MBA) students only
 */
export async function POST(req) {
  try {
    ...
    if (userRole === 'user' || userRole === 'student') {
      ...
      console.log(`Access granted to ${userRole}: ${payload.email} accessing SOM student data`);
    } else {
      return NextResponse.json({
    ...
    if (registration) {
      // Parse registration using SOM parser
      const { parseSOMRegistration } = await import('@/app/api/som/parse-registration/route');
      const parsed = parseSOMRegistration(registration);

      // Verify SOM (BBA/MBA) student
      if (!parsed || !parsed.isValid || !parsed.isSOM) {
        return NextResponse.json({
          error: "This route is for SOM (BBA/MBA) students only"
        }, { status: 400 });
      }
      ...
      return NextResponse.json({ records, school: 'SOM' });
    }
    ...
    if (department || batch) {
      ...
      if (department && department !== "All") {
        const searchKey = department.trim().toLowerCase();
        let targetBranch = null;
        if (searchKey === 'bba' || searchKey.includes('business administration')) targetBranch = "Bachelor of Business Administration";
        else if (searchKey === 'mba' || searchKey.includes('master of business')) targetBranch = "Master of Business Administration";
      ...
      if (department && department !== "All") {
        ...
        const somBranchCodes = {
          'BBA': '912',
          'MBA': '214',
          'Bachelor of Business Administration': '912',
        ...
        if (somBranchCodes[department]) {
          code = somBranchCodes[department];
        }
        else if (deptUpper.includes('BBA') || deptUpper.includes('BUSINESS ADMINISTRATION')) {
          code = '912';
        }
        else if (deptUpper.includes('MBA') || deptUpper.includes('MANAGEMENT')) {
          code = '214';
        }

        if (code) {
          // SOM (BBA/MBA) branch code is at index 5-7. preceded by 5 chars (YY, II, P).
          // Regex: ^.{5}912...
          branchQueries.push({ Reg_No: { $regex: `^.{5}${code}` } });
        } else {
          if (deptUpper === 'BBA') branchQueries.push({ Reg_No: { $regex: `^.{5}912` } });
          if (deptUpper === 'MBA') branchQueries.push({ Reg_No: { $regex: `^.{5}214` } });
        }
      ...
      const students = await db.collection("som_result").find(query).project({
        _id: 0,
        Reg_No: 1,
        Name: 1,
        Branch: 1
      ...
      // Filter for SOM (BBA/MBA) students AND apply efficient filtering
      const { parseSOMRegistration } = await import('../parse-registration/route');
      const btechStudents = [];
      students.forEach(student => {
        const parsed = parseSOMRegistration(student.Reg_No);
        if (!parsed || !parsed.isValid || !parsed.isSOM) return;
      ...
      return NextResponse.json({ students: uniqueStudents, school: 'SOM' });
    }

    return NextResponse.json({ error: "registration or department required" }, { status: 400 });
  } catch (err) {
    console.error("/api/som/students POST error", err);
...
export async function PUT(req) {
  try {
    ...
    console.error("/api/som/students PUT error", err);
    return NextResponse.json({ error: "Server error" }, { status: 500 });
  }
}
...
export async function DELETE(req) {
  try {
    ...
    console.error("/api/som/students DELETE error", err);
    return NextResponse.json({ error: "Server error" }, { status: 500 });
  }
}
