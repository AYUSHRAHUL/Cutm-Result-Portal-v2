...
/**
 * SOM Analytics Subject Students Route - SOM (BBA/MBA) only
...
export async function GET(req) {
  try {
    ...
    console.log(`[SOM Subject Students] Database selection: campus=${campus}, school=${school}, dbName=${dbName}`);
    
    const db = client.db(dbName);
    const cutm = db.collection("som_result");

    // Filter for SOM (BBA/MBA) students only (parser used after DB query)
