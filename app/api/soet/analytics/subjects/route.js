...
/**
 * SOET Analytics Subjects Route - B.Tech only
 */
  ...
  try {
    ...
    if (branchFilter && branchFilter !== "all") {
      ...
      if (branchCodes.length > 0) {
        const singleDigitCodes = branchCodes.filter(c => c.length === 1);
        const twoDigitCodes = branchCodes.filter(c => c.length === 2);

        if (singleDigitCodes.length > 0) {
          orConditions.push({
            Reg_No: { $regex: `^.{7}[${singleDigitCodes.join('')}]` }
          });
        }
        if (twoDigitCodes.length > 0) {
          const twoDigitRegex = twoDigitCodes.map(code => `^.{6}${code}`).join('|');
          orConditions.push({
            Reg_No: { $regex: twoDigitRegex }
          });
        }
      }
    ...
    console.error('SOET Subjects API error:', error);
    return NextResponse.json({
