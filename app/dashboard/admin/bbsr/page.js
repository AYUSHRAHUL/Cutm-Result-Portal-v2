...
export default function AdminDashboardBBSR() {
  ...
  useEffect(() => {
    ...
    const school = schoolParam || savedSchool;
    if (school) {
    ...
    return (
      ...
      >
        ...
        <div className="text-white/90 text-sm sm:text-base text-center px-4 font-semibold">
          🔐 Secure Admin Access - BBSR Campus
        </div>
    ...
    >
      ...
      <section className="pt-12 sm:pt-16 pb-6 sm:pb-8 text-center px-3 sm:px-6">
        <div className="mx-auto max-w-5xl">
          ...
          >
            CUTM BBSR - Admin Dashboard
          </h1>
          <p className="text-[#5A6C7D] text-sm sm:text-base font-medium">
            Manage and monitor your academic platform - BBSR Campus
          </p>
        </div>
      </section>
      ...
      <section className="py-6 sm:py-8 px-3 sm:px-6">
        <div className="mx-auto max-w-6xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 mb-8 sm:mb-12">
            <SchoolCard
              title="SOET"
              subtitle="School of Engineering & Technology"
              icon="🏗️"
              isSelected={selectedSchool === 'soet'}
              onClick={() => {
                setSelectedSchool('soet');
                localStorage.setItem('selectedSchool', 'soet');
                localStorage.setItem('selectedCampus', 'bbsr');
                router.push('/dashboard/admin/bbsr/soet');
              }}
            ...
            {/* <SchoolCard
              ...
              icon="💼"
              isSelected={selectedSchool === 'som'}
              onClick={() => {
                setSelectedSchool('som');
                localStorage.setItem('selectedSchool', 'som');
                localStorage.setItem('selectedCampus', 'bbsr');
                router.push('/dashboard/admin/bbsr/som');
              }}
            ...
            <SchoolCard
              ...
              icon="🎓"
              isSelected={selectedSchool === 'sovet'}
              onClick={() => {
                setSelectedSchool('sovet');
                localStorage.setItem('selectedSchool', 'sovet');
                localStorage.setItem('selectedCampus', 'bbsr');
                router.push('/dashboard/admin/bbsr/sovet');
              }}
      ...
      {/* No modules on campus page - only school selection */}
      {/* Modules will be shown on a separate page after school selection */}
...
function SchoolCard({ title, subtitle, icon, onClick, gradient, isSelected }) {
  return (
    ...
    >
      {isSelected && (
        <div className="absolute top-4 right-4 bg-white text-blue-600 px-3 py-1 rounded-full text-sm font-bold z-20">
          Selected
        </div>
      )}
