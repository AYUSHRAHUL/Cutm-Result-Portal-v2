...
function TeacherDashboardBBSRContent() {
  ...
  useEffect(() => {
    ...
    console.log('[BBSR DASHBOARD] School from URL:', school);
    console.log('[BBSR DASHBOARD] Search params:', Object.fromEntries(searchParams.entries()));
    
    if (school) {
      setSelectedSchool(school);
      localStorage.setItem('selectedSchool', school);
      localStorage.setItem('campus', 'bbsr');
      console.log('[BBSR DASHBOARD] School stored:', school);
    } else {
      // Try to get from localStorage
      const savedSchool = localStorage.getItem('selectedSchool');
      console.log('[BBSR DASHBOARD] No school in URL, checking localStorage:', savedSchool);
      if (!savedSchool) {
        // No school selected, redirect back to school selection
        console.warn('[BBSR DASHBOARD] No school selected anywhere, redirecting to selection');
    ...
    return (
      ...
      >
        ...
        <div className="text-white/90 text-sm sm:text-base text-center px-4 font-semibold flex items-center gap-2">
          ...
          <span>CUTM BBSR - Teacher Portal</span>
        </div>
    ...
    >
      ...
      <div className="fixed top-0 left-0 right-0 h-1 sm:h-1.5 z-50">
        <div 
          ...
          style={{
            background: "linear-gradient(90deg, #8B5CF6 0%, #F18F01 50%, #6D28D9 100%)",
            opacity: 0.6
          }}
        />
      </div>
        ...
        <div className="mx-auto max-w-6xl">
          <div className="flex flex-col items-center justify-center space-y-3 sm:space-y-4">
            ...
            <p className="text-xs sm:text-sm md:text-base text-[#5A6C7D] font-medium text-center max-w-2xl px-4">
              Manage student results, review backlogs, and explore batch data for BBSR Campus
            </p>
          </div>
        </div>
      </section>
...
function ModuleCard({ title, icon, gradient, children, onClick }) {
  return (
    <button 
      onClick={onClick} 
      className="group text-left rounded-xl sm:rounded-2xl border-2 bg-white p-4 sm:p-5 lg:p-6 transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl focus:outline-none focus:ring-4 focus:ring-[#8B5CF6]/20 relative overflow-hidden"
      style={{
    ...
    >
      ...
      {/* Title */}
      <h4 className="text-base sm:text-lg font-black text-[#1A1F29] text-center mb-2 sm:mb-3 group-hover:text-[#8B5CF6] transition-colors">
        {title}
      </h4>
      ...
      >
        <li className="flex items-center gap-2 text-[#1A1F29]">
          ...
          <span className="group-hover:text-[#8B5CF6] transition-colors font-medium">Quick Access</span>
        </li>
        <li className="flex items-center gap-2 text-[#1A1F29]">
          <span className="text-green-500 group-hover:scale-125 transition-transform text-sm">✓</span> 
          <span className="group-hover:text-[#8B5CF6] transition-colors font-medium">Real-time Data</span>
        </li>
      </ul>
      ...
      <div className="flex justify-center">
        <span 
          ...
          style={{
            background: "linear-gradient(135deg, rgba(139,92,246,0.1) 0%, rgba(241,143,1,0.1) 100%)",
            color: "#8B5CF6",
...
export default function TeacherDashboardBBSR() {
  return (
    <Suspense fallback={<div className="p-6 text-center">Loading...</div>}>
      <TeacherDashboardBBSRContent />
    </Suspense>
  );
}
